...
from utils import get_regex_from_normalization_rule_name
...
byte_encoder = {v: k for k, v in bytes_to_unicode().items()}


def token_string_to_bytes(s: str) -> bytes:
    return bytes([byte_encoder[c] for c in s])
...
def train(args: TrainBPETokenizerArgs):
    ...
    trainer = BpeTrainer(
        ...
        show_progress=True,
        special_tokens=[
            SPECIAL_TOKENS["bos_piece"],
            SPECIAL_TOKENS["eos_piece"],
            SPECIAL_TOKENS["pad_piece"],
            SPECIAL_TOKENS["fim_prefix"],
            SPECIAL_TOKENS["fim_middle"],
            SPECIAL_TOKENS["fim_suffix"],
        ]
        + [f"<0x{i:02X}>" for i in range(256)],  # seed sm vocab
        max_token_length=args.max_sentencepiece_length,
    )

    if "identity" in args.normalization_rule_name:
        # byte level
    ...
    if args.output_name is None:
        args.output_name = generate_model_name(asdict(args))
    ...
    filename = f"{args.output_dir.rstrip('/')}/{args.output_name}.json"
    tokenizer.save(filename)
    logging.info(
        f"Training terminated successfully, tokenizer path saved to:\n {filename}"
    )
